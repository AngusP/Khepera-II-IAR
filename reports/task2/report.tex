...
Several methods of odometry were researched and evaluated. Firstly, several methods of
 calculating the pose ${(X,Y,\Theta)}$. The first one we found was the one we ended up using
...
used a fourth order Runge-Kutta numerical integrator\cite{runge_kutta}. However, upon implementation, and
...
that the data given in the Khepera 2 User Manual\cite{khepera_manual} redgarding each milimiter distance 
being equivalent to $12$ encoder ticks.
